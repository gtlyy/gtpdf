...
static void* tesseractHandle = NULL;
...
int loadTesseractLib(const char* path) {
	tesseractHandle = dlopen(path, RTLD_NOW | RTLD_GLOBAL);
	if (!tesseractHandle) return 0;

	pCreate     = (tess_create_t)dlsym(tesseractHandle, "TessBaseAPICreate");
	pInit       = (tess_init_t)dlsym(tesseractHandle, "TessBaseAPIInit3");
	pSetImage   = (tess_set_image_t)dlsym(tesseractHandle, "TessBaseAPISetImage");
	pSetRect    = (tess_set_rect_t)dlsym(tesseractHandle, "TessBaseAPISetRectangle");
	pGetText    = (tess_get_text_t)dlsym(tesseractHandle, "TessBaseAPIGetUTF8Text");
	pDelete     = (tess_delete_t)dlsym(tesseractHandle, "TessBaseAPIDelete");
	pDeleteText = (tess_delete_text_t)dlsym(tesseractHandle, "TessDeleteText");
	pRecognize  = (tess_recognize_t)dlsym(tesseractHandle, "TessBaseAPIRecognize");
	pSetPageSegMode = (tess_set_psm_t)dlsym(tesseractHandle, "TessBaseAPISetPageSegMode");
